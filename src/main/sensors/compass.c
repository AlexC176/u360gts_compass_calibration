...
extern void setPanInverted(bool invert); // main.c
...
void updateCompass(flightDynamicsTrims_t *magZero)
{
    ...
    static float    signed_sum_vec_angle=0;	// detect rotation direction
	
	static float    vec_angle_p01=0;
	static float    vec_angle_p12=0;
	static float    delta_angle=0;
	static float 	signed_delta_angle=0; // detect rotation direction
    static float    dist=0;
    ...
    if(detectedSensors[SENSOR_INDEX_MAG] != MAG_NONE && magInit)
	{
		...
		if (tCal != 0) {
            ...
            if ( adc_sample > ADC_SAMPLE_P1 )
            {
				...
				// delta angle always positive, regardless of spin direction
				...
				printf(" da=%s", buf); // delta angle
				//printf(" using Vector Angle p01 =%f\n", vec_angle_p01);
				
                signed_delta_angle = fmodf(vec_angle_p01 - vec_angle_p12 + 540.0f, 360.0f) - 180.0f;
                npf_snprintf(buf, sizeof(buf),"%f",signed_delta_angle);
				printf(" sda=%s\n", buf); // signed delta angle
                ...
                if ( delta_angle  > MAG_ANGLE_THRESHOLD && dist > MAG_DISTANCE_THRESHOLD )
                {
                    ...
                    if ( evSample > 0 ) {
						...
						signed_sum_vec_angle += signed_delta_angle;
					}
				    ...
				    printf("sum_vec_angle=%s,", buf);
					npf_snprintf(buf, sizeof(buf), "%f",signed_sum_vec_angle);
				    printf("signed_sum_vec_angle=%s\n", buf);
			...
			else 
			{
				...
				{ 
				  ...
				  // Aug 2026 - if calibration matrix made it worse, then reset
			      if ( sd2 >= sd1 )
				  {
				  softIron[0][0] = 1.0f;  
			      softIron[0][1] = 0.0f;
				  ...
				  // should rotate anti-clockwise (positive) during calibration
				  // direction critical for PID and noPID code
				  if (signed_sum_vec_angle < 0.0f) {
				      setPanInverted(true);  // reverse clockwise spin
				  } 
				  else {
					  setPanInverted(false); // ensure its off
				  }
										
				  // 12 seconds hold time
				  notifyUpdateAt = currentTime + 12000000;
